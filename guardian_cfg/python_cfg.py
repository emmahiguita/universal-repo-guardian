...
def build_cfg(path: Path):
    ...
    def new(kind,line,label):
        nonlocal idx
        idx+=1; nodes.append({"id":idx,"kind":kind,"line":line,"label":label}); return idx
    for fn in [n for n in ast.walk(tree) if isinstance(n,(ast.FunctionDef,ast.AsyncFunctionDef))]:
        start=new("function",fn.lineno,fn.name)
        prev=start
        for stmt in fn.body:
            cur=new(type(stmt).__name__,getattr(stmt,"lineno",fn.lineno),ast.dump(stmt,include_attributes=False)[:180])
            edges.append({"from":prev,"to":cur,"kind":"NEXT"}); prev=cur
        ...
        edges.append({"from":prev,"to":idx+1,"kind":"NEXT"}); idx+=1
